...
// Upload a story (photo OR video). Photos are watermarked; videos stored
// as-is. Returns { url, media_type }.
export async function uploadStory(file, talentId) {
  const type = (file?.type || "").toLowerCase();
  const pub = (path) => supabase.storage.from("talent-photos").getPublicUrl(path).data.publicUrl;
  if (type.startsWith("image/")) {
    validateImageFile(file);
    const wm = await applyWatermark(file);
    const name = `${talentId}/story_${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
    const { data, error } = await supabase.storage
      .from("talent-photos").upload(name, wm, { cacheControl: "86400", upsert: false });
    if (error) throw error;
    return { url: pub(data.path), media_type: "photo" };
  }
  if (type.startsWith("video/")) {
    validateVideoFile(file);
    const ext = (file.name.split(".").pop() || "mp4").toLowerCase().replace(/[^a-z0-9]/g, "");
    const name = `${talentId}/story_${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
    const { data, error } = await supabase.storage
      .from("talent-photos").upload(name, file, { cacheControl: "86400", upsert: false, contentType: file.type });
    if (error) throw error;
    return { url: pub(data.path), media_type: "video" };
  }
